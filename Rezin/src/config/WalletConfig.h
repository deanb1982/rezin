...
namespace WalletConfig
{
    ...
    const std::string_view addressPrefix = "RZCC";

    /* Your coins 'Ticker', e.g. Monero = XMR, Bitcoin = BTC */
    const std::string ticker = "RZCC";
    ...
    const std::string daemonName = "Rezind";

    /* The name to call this wallet */
    const std::string walletName = "Rezin-Wallet";
    ...
    const std::string contactLink = "https://discord.gg/TeanRezin";
    ...
    /* The default fee value to use with transactions (in ATOMIC units!) - 500 NLM */
    const uint64_t defaultFee = CryptoNote::parameters::MINIMUM_FEE;

    /* The minimum fee value to allow with transactions (in ATOMIC units!)  - 500 NLM */
